for (var i = 0; i < window.platform.length; i++) {
    eval("var " + window.platform[i].name + " = " + window.platform[i].name + " || [];");
    eval(window.platform[i].name + ".push(['_setAccount','" + window.platform[i].id + "']);")
}
(function () {
    ...
    window.setShareData = function () {
        wx.ready(function () {
            var shareFriendSuccessFunc = function () {
                window.statistics(window.platform, "to", window.h6app_shareConfig.toFriend || "好友和群组");
            ...
            var shareTimelineSuccessFunc = function () {
                window.statistics(window.platform, "to", window.h6app_shareConfig.toTimeLine || "朋友圈");
                h6app_shareConfig.success()
            };
    ...
    function pvStatistics(platform, key, value) {
        ...
        if (location.href.indexOf("from=groupmessage") > 0) {
            window.statistics(window.platform, "from", window.h6app_shareConfig.fromGroup || "来自微信群")
        } else {
            if (location.href.indexOf("from=singlemessage") > 0) {
                window.statistics(window.platform, "from", window.h6app_shareConfig.fromSingle || "来自好友")
            } else {
                if (location.href.indexOf("from=timeline") > 0) {
                    window.statistics(window.platform, "from", window.h6app_shareConfig.fromTimline || "来自朋友圈")
                }
            }
        }
    }
